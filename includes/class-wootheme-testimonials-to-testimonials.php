...
class Wootheme_Testimonials_to_Testimonials extends Aihrus_Common {
	...
	public static function admin_init() {
		...
		self::$settings_link = '<a href="' . get_admin_url() . 'edit.php?post_type=' . Axl_Testimonials_Widget::PT . '&page=' . Wootheme_Testimonials_to_Testimonials_Settings::ID . '">' . __( 'Settings', 'wootheme-testimonials-to-testimonials' ) . '</a>';
	}


	public static function admin_menu() {
		self::$menu_id = add_submenu_page( 'edit.php?post_type=' . Axl_Testimonials_Widget::PT, esc_html__( 'WooTheme Testimonials to Testimonials Migrator', 'wootheme-testimonials-to-testimonials' ), esc_html__( 'WTT Migrator', 'wootheme-testimonials-to-testimonials' ), 'manage_options', self::ID, array( __CLASS__, 'user_interface' ) );
		...
		add_screen_meta_link(
			...
			admin_url( 'edit.php?post_type=' . Axl_Testimonials_Widget::PT . '&page=' . Wootheme_Testimonials_to_Testimonials_Settings::ID ),
			self::$menu_id,
	...
	public static function plugin_action_links( $links, $file ) {
		if ( self::BASE == $file ) {
			...
			$link = '<a href="' . get_admin_url() . 'edit.php?post_type=' . Axl_Testimonials_Widget::PT . '&page=' . self::ID . '">' . esc_html__( 'Migrate', 'wootheme-testimonials-to-testimonials' ) . '</a>';
...
<?php
	...
	public static function ajax_process_post() {
		...
		if ( ! $post || ! in_array( $post['post_type'], self::$post_types )  ) {
			die( wp_json_encode( array( 'error' => sprintf( esc_html__( 'Failed Migration: %s is incorrect post type.', 'wootheme-testimonials-to-testimonials' ), esc_html( self::$post_id ) ) ) ) );
		}

		$result = self::migrate_item( self::$post_id, $post );
		if ( is_numeric( $result ) ) {
			die( wp_json_encode( array( 'success' => sprintf( __( '&quot;<a href="%1$s" target="_blank">%2$s</a>&quot; WooTheme Testimonial ID %3$s was successfully migrated to Testimonials %6$s &quot;<a href="%4$s" target="_blank">%5$s</a>&quot;.', 'wootheme-testimonials-to-testimonials' ), get_permalink( self::$post_id ), esc_html( get_the_title( self::$post_id ) ), self::$post_id, get_permalink( $result ), esc_html( get_the_title( $result ) ), $result ) ) ) );
		} else {
			die( wp_json_encode( array( 'error' => sprintf( __( '&quot;<a href="%1$s" target="_blank">%2$s</a>&quot; Unable to be migrated.', 'wootheme-testimonials-to-testimonials' ), get_permalink( self::$post_id ), esc_html( get_the_title( self::$post_id ) ) ) ) ) );
		}
	}
